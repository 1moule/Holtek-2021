  ...
  for(i=0; i<info->num; i++)//每次循环对应事件队列中的一个事件
  {
    switch(info->event[i])//事件队列中的第i个事件
    {
      ...
      case EVENT_Light://app点击补光系统开关
			...
			if(0x01 == currentDataPoint.valueLight)//值为1
        {
          //user handle 这里调用自己写的led函数
				  GPIO_SetBits(GPIOA,GPIO_Pin_0);						 //PC13 输出高
        ...
        {
          //user handle  
					GPIO_ResetBits(GPIOA,GPIO_Pin_0);						 //PC13 输出低
      ...
      case EVENT_water://灌溉系统
        ...
        {
					GPIO_SetBits(GPIOA,GPIO_Pin_3);//设置比较值        开水
          //user handle
        }
        else
        {
					GPIO_ResetBits(GPIOA,GPIO_Pin_3);//设置比较值			关水
          //user handle    
        }
        break;
     ...
     case EVENT_windows:
		    ...
		    if(0x01 == currentDataPoint.valuewindows)
        {
					...
					//delay_ms(200);
					TIM_SetCompare3(TIM2,190);//设置比较值   开窗
          //user handle
        }
        else
        {
					...
					//delay_ms(200);
					TIM_SetCompare3(TIM2,185);//设置比较值   关窗
          //user handle    
        }
        break;
