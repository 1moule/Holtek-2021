...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagLight)
    {
        info->event[info->num] = EVENT_Light;
        info->num++;
        dataPoints->valueLight = gizStandardDecompressionValue(Light_BYTEOFFSET,Light_BITOFFSET,Light_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagwater)
    {
        info->event[info->num] = EVENT_water;
        info->num++;
        dataPoints->valuewater = gizStandardDecompressionValue(water_BYTEOFFSET,water_BITOFFSET,water_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagwindows)
    {
        info->event[info->num] = EVENT_windows;
        info->num++;
        dataPoints->valuewindows = gizStandardDecompressionValue(windows_BYTEOFFSET,windows_BITOFFSET,windows_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuewindows != cur->valuewindows)
    {
        GIZWITS_LOG("valuewindows Changed\n");
        ret = 1;
    }
    ...
    if(last->valueLightintensity != cur->valueLightintensity)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueLightintensity Changed\n");
            ret = 1;
        }
    }
    if(last->valuesoil_moisture != cur->valuesoil_moisture)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesoil_moisture Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Light_BYTEOFFSET,Light_BITOFFSET,Light_LEN,(uint8_t *)devStatusPtr,dataPoints->valueLight);
    gizStandardCompressValue(water_BYTEOFFSET,water_BITOFFSET,water_LEN,(uint8_t *)devStatusPtr,dataPoints->valuewater);
    gizStandardCompressValue(windows_BYTEOFFSET,windows_BITOFFSET,windows_LEN,(uint8_t *)devStatusPtr,dataPoints->valuewindows);
    ...
    devStatusPtr->valueLightintensity = gizY2X(Lightintensity_RATIO,  Lightintensity_ADDITION, dataPoints->valueLightintensity); 
    devStatusPtr->valuesoil_moisture = gizY2X(soil_moisture_RATIO,  soil_moisture_ADDITION, dataPoints->valuesoil_moisture); 
